...
ip link add link ${phy} ${phy}.20 type vlan id 20
ip addr add ${phyipmask} dev ${phy}.20
ip link set ${phy} up
ip link set ${phy}.20 up
...
ip link add link ${uplinkphy} ${uplinkphy}.10 type vlan id 10
ip addr add ${uplinkipmask} dev ${uplinkphy}.10
ip link set ${uplinkphy} up
ip link set ${uplinkphy}.10 up

phyip=$(echo $phyipmask | cut -d'/' -f1)

cat $OspfTemplateFile | sed s/%OSPF-ROUTER-ID%/${phyip}/g >> /etc/quagga/ospfd.conf
cat $BgpTemplateFile | sed s/%BGP-ROUTER-ID%/${phyip}/g | sed s/%BGP-NORTH-NEIGHBOR%/${nexthop}/g >> /etc/quagga/bgpd.conf
...
iptables -t mangle -A FORWARD -i ${uplinkphy}.10 -m dscp --dscp-class EF -j CLASSIFY --set-class 1:7
iptables -t mangle -A FORWARD -i ${uplinkphy}.10 -m dscp --dscp-class EF -j MARK --set-mark 0x7
...
iptables -t mangle -A FORWARD -i ${uplinkphy}.10 -m dscp --dscp-class AF21 -j CLASSIFY --set-class 1:3
iptables -t mangle -A FORWARD -i ${uplinkphy}.10 -m dscp --dscp-class AF21 -j MARK --set-mark 0x3
